	...
	return new Promise((resolve, reject) => {
		fetch('https://jagokata.com/kata-bijak/kata-' + input.replace(/\s/g, '_') + '.html?page=1')
			...
			.then(res => {
				...
				data.splice(2, 1)
				if (data.length == 0) return resolve({ creator: '@neoxr - Wildan Izzudin & @ariffb.id - Ariffb. Recode : Andi', status: false })
				resolve({ creator: '@neoxr - Wildan Izzudin & @ariffb.id - Ariffb. Recode : Andi', status: true, data })
			}).catch(reject)
	})
}